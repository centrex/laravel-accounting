...
@php
    ...
    // 1. Bill posting JE
    // 2. Payment JEs
    // 3. Charge / Discount JEs (from expenses)
    $allJournals = collect();
    ...
    foreach ($bill->expenses as $charge) {
...
<div class="mt-4 mb-4 grid gap-4 xl:grid-cols-3">
    ...
    <div class="flex flex-col gap-4">
                    ...
                    <div class="px-5 py-3">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="text-right">
                                <div class="font-mono font-semibold text-sm">{{ $bill->base_currency }} {{ number_format($bill->convertToBase($payment->amount), 2) }}</div>
                                @if($payment->journalEntry)
        ...
        {{-- Charges & Discounts --}}
        @if($bill->expenses->isNotEmpty())
        <x-tallui-card padding="none">
            <div class="border-b border-base-200 px-5 py-4">
                <div class="flex items-center justify-between">
                    <h3 class="font-semibold">Charges & Discounts</h3>
                    <span class="badge badge-ghost badge-sm">{{ $bill->expenses->count() }}</span>
                </div>
            </div>
            <div class="divide-y divide-base-200">
                @foreach($bill->expenses as $item)
                    ...
                    <div class="px-5 py-3">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <x-tallui-badge :type="$itemBadge" size="sm">{{ $item->account?->name ?? 'Entry' }}</x-tallui-badge>
                                <div class="mt-1 text-xs text-base-content/60">{{ $item->expense_date?->format('M d, Y') }}</div>
                                @if($item->notes)
...
<x-tallui-card class="mt-4" padding="none">
    <div class="border-b border-base-200 px-5 py-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-base-content/60">All accounting entries for this bill</p>
            </div>
    ...
    @empty
        <div class="px-5 py-6 text-sm text-base-content/60">No journal entries recorded for this bill yet.</div>
    @endforelse
</x-tallui-card>
...
<div class="modal modal-open">
    <div class="modal-box max-w-md">
        <h3 class="mb-4 text-lg font-bold">Record Purchase Discount</h3>
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-lg border border-base-200 bg-base-50 px-4 py-3 text-sm">
                <div class="flex items-center justify-between">
                    <span class="text-base-content/60">Available AP balance</span>
                    <span class="font-mono font-semibold {{ $this->availableAp() > 0 ? 'text-success' : 'text-error' }}">
                        {{ $bill->base_currency }} {{ number_format($this->availableAp(), 2) }}
                    </span>
                </div>
            </div>
