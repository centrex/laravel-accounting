...
<div class="hidden print:block mb-6 border-b pb-4">
    <h1 class="text-xl font-bold">Supplier Ledger — Statement of Account</h1>
    <p class="text-base font-medium mt-1">{{ $vendor->name }} ({{ $vendor->code }})</p>
...
<x-tallui-card class="mb-4">
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div>
            ...
            <div class="text-sm font-medium">{{ $vendor->email ?? '—' }}</div>
        </div>
        <div>
        ...
        <div>
            ...
            <div class="text-sm font-medium">{{ $vendor->currency }}</div>
        </div>
        <div>
            <div class="text-xs uppercase text-base-content/50">Payment Terms</div>
            <div class="text-sm font-medium">{{ $vendor->payment_terms }} days</div>
        </div>
    </div>
</x-tallui-card>
...
<x-tallui-card padding="none">
    <div class="border-b border-base-200 px-5 py-4 flex items-center justify-between">
        ...
        <div class="text-right">
            <div class="text-xs uppercase text-base-content/50">Outstanding Payable</div>
            <div class="text-lg font-mono font-bold {{ $ledger['closing'] > 0 ? 'text-warning' : ($ledger['closing'] < 0 ? 'text-error' : 'text-success') }}">
                {{ $vendor->currency }} {{ number_format($ledger['closing'], 2) }}
            </div>
        </div>
    </div>
        ...
        <table class="table table-sm w-full">
            <thead>
                <tr class="bg-base-50 text-xs uppercase text-base-content/50">
                    ...
                    <th>Description</th>
                    <th class="text-right w-32">Billed</th>
                    <th class="text-right w-32">Paid</th>
