...
trait AddTablePrefix
{
    ...
    {
        return self::accountingSchemaSettings()['prefix'] . $this->getTableSuffix();
    }
    ...
    {
        return $this->connection ?? self::accountingSchemaSettings()['connection'];
    }

    /**
     * @return array{prefix: string, connection: string|null}
     */
    private static function accountingSchemaSettings(): array
    {
        ...
        if (!isset(self::$addTablePrefixSettings[$key])) {
            $prefix = $config->get('accounting.table_prefix');
            $connection = $config->get('accounting.drivers.database.connection')
                ?? $config->get('database.default');

            self::$addTablePrefixSettings[$key] = [
                'prefix'     => is_string($prefix) && $prefix !== '' ? $prefix : 'acct_',
